...
CREATE TABLE IF NOT EXISTS courses (
  ...
  semester INTEGER,
  syllabus_version VARCHAR(20) NOT NULL DEFAULT '1.0',
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS admission_programs (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (admission_cycle_id, program_id)
);

CREATE TABLE IF NOT EXISTS applications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  applicant_user_id UUID NOT NULL REFERENCES users(id) ON DELETE RESTRICT,
  admission_program_id UUID NOT NULL REFERENCES admission_programs(id) ON DELETE RESTRICT,
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (applicant_user_id, admission_program_id)
);
...
CREATE TABLE IF NOT EXISTS facility_pricing_rules (
  ...
  requester_role_code VARCHAR(50) NOT NULL,
  event_type VARCHAR(100) NOT NULL,
  base_price NUMERIC(12,2) NOT NULL,
  per_hour_price NUMERIC(12,2),
  is_active BOOLEAN NOT NULL DEFAULT TRUE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (facility_id, requester_role_code, event_type)
);
...
CREATE TABLE IF NOT EXISTS tenders (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tender_no VARCHAR(100) NOT NULL UNIQUE,
  title TEXT NOT NULL,
  description TEXT,
...
CREATE INDEX IF NOT EXISTS idx_applications_admission_program_id ON applications(admission_program_id);
